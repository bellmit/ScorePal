...
void MacAddress::read(uint8_t word, uint8_t loc) {
...
void MacAddress::getAddress(uint8_t* buffer) {
	for (uint8_t i = 0; i < 6; ++i) {
		buffer[i] = m[i];
	}
}

void MacAddress::printToSerial() {
	for(uint8_t i = 0; i < 6; ++i) {
		if (i != 0) {
			Serial.print(F(":"));
		}
		Serial.print((*(m+i) & 0xF0) >> 4, 16);
		Serial.print((*(m+i) & 0x0F), 16);
	}
}
