...
MaxMatrix::MaxMatrix(byte _data, byte _load, byte _clock, byte _num) 
{
	...
	clock = _clock;
	num = _num;
	for (int i = 0; i < max7219_bufferSize; i++)
		buffer[i] = 0;
}
	...
	if (h == 8 && y == 0)
		...
		for (int i=0; i<w; i++)
			for (int j=0; j<h; j++)
			{
				int c = x + i;
				int r = y + j;
				if (c>=0 && c<max7219_bufferSize && r>=0 && r<8)
...
void MaxMatrix::shiftLeft(bool rotate, bool fill_zero)
{
	byte old = buffer[0];
	int i;
	for (i=0; i<max7219_bufferSize; i++)
		buffer[i] = buffer[i+1];
