...
public class KeyController extends ButtonController {
    ...
    protected boolean processButtonPress(ControllerButton button, ArrayList<Long> clickTimes, KeyEvent keyEvent) {
        ...
        if (keyEvent.isLongPress()) {
            ...
            if (keyEvent.getDownTime() == lastLongKeyDownTime) {
                ...
                // inform listeners of this press for some visual feedback
                informListeners(new KeyPress[] {KeyPress.Short});
            }
        }
    ...
    protected boolean processLongButtonPress(ControllerButton button, long downTime) {
        ...
        informListeners(new KeyPress[] {KeyPress.Long});
        ...
        for (Pair<ControllerPattern, ControllerAction> action : button.actions) {
            if (action.first == ControllerPattern.LongClick) {
                ...
                informListeners(button.codes, action.first, action.second);
            }
        }
    ...
    private void processClickTimes() {
        while (this.isProcessClickTimes) {
            ...
            if (buttons.length == safeClickTimes.length) {
                ...
                for (int i = 0; i < buttons.length; ++i) {
                    ...
                    if (clickCount > 0) {
                        ...
                        informListeners((KeyPress[])null);
                        for (Pair<ControllerPattern, ControllerAction> action : buttons[i].actions) {
                            // for each action, is this ok?
                            switch (action.first) {
                                case SingleClick:
                                    if (clickCount == 1) {
                                        informListeners(buttons[i].codes, action.first, action.second);
                                    }
                                    break;
                                case DoubleClick:
                                    if (clickCount == 2) {
    ...
    protected void informListeners(int[] keyCodes, ControllerPattern pattern, ControllerAction action) {
        // just delegate to the action for this key

        //!!! we are to use the pattern as the action is determined when this controller is created
        // the pattern on the other hand can change as the user changes the settings (one click to this
        // and two do that etc)
        informListeners(pattern);
    }
}
