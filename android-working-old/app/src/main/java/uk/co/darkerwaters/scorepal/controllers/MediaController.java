...
public class MediaController extends KeyController {
    ...
    public MediaController(SettingsControl appSettings, Context context) {
        super(GetControllerButtons(appSettings));
    ...
    public static ControllerButton[] GetControllerButtons(SettingsControl appSettings) {
        // we are either doing team one / team two, or we are doing server / receiver
        ControllerAction actionOne = appSettings.getIsControlTeams() ? ControllerAction.PointTeamOne : ControllerAction.PointServer;
        ControllerAction actionTwo = appSettings.getIsControlTeams() ? ControllerAction.PointTeamTwo : ControllerAction.PointReceiver;
        // and create the buttons for this
        List<ControllerButton> buttons = new ArrayList<ControllerButton>();
        /*if (appSettings.getIsControlUseFlic()) {
            // flic doesn't need buttons, it is intercepted and handled directly from the receiver
            // to the communicator
        }*/
        if (appSettings.getIsControlUseMedia() || appSettings.getIsControlUsePuck()) {
            // add the media buttons (puck is programmed to use the same messages as these)
        ...
        if (appSettings.getIsControlUseVol()) {
    ...
    public void setButtons(SettingsControl appSettings) {
        setButtons(GetControllerButtons(appSettings));
    }

    public boolean start(SettingsControl appSettings) {
