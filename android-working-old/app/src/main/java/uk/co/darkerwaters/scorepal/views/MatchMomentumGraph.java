...
public class MatchMomentumGraph extends View {
    ...
    private static final float K_AXIS_WIDTH_DP = 2f;
    private static final float K_LINE_WIDTH_DP = 1f;
    private static final float K_IMPR_WIDTH_DP = 3f;
    private static final float K_DOT_WIDTH_DP = 5f;
    ...
    private final List<HistoryValue> pointHistory = new ArrayList<>();
    ...
    private int teamOnVerticalAxis = 0;
    ...
    public MatchMomentumGraph(Context context, AttributeSet attrs) {
        ...
        this.backgroundPaint.setColor(ContextCompat.getColor(context, R.color.colorPrimary));
        this.backgroundPaint.setAntiAlias(true);

        this.axisPaint = new Paint();
        this.axisPaint.setStyle(Paint.Style.STROKE);
        this.axisPaint.setColor(ContextCompat.getColor(context, R.color.colorPrimaryDark));
        this.axisPaint.setAntiAlias(true);
        this.axisPaint.setStrokeWidth(BaseActivity.convertDpToPixel(K_AXIS_WIDTH_DP, context));

        this.linePaint = new Paint();
        this.linePaint.setStyle(Paint.Style.STROKE);
        this.linePaint.setAntiAlias(true);
        this.linePaint.setStrokeWidth(BaseActivity.convertDpToPixel(K_LINE_WIDTH_DP, context));

        this.textPaint = new Paint();
        this.textPaint.setStyle(Paint.Style.FILL);
        this.textPaint.setColor(ContextCompat.getColor(context, R.color.colorSecondary));
        this.textPaint.setAntiAlias(true);
        this.textPaint.setTextAlign(Paint.Align.LEFT);
        this.textPaint.setTextSize(BaseActivity.convertDpToPixel(K_POINT_SEP_DP, context));

        this.dotPaint = new Paint();
        this.dotPaint.setStyle(Paint.Style.FILL);
        this.dotPaint.setAntiAlias(true);

        this.dotRadius = BaseActivity.convertDpToPixel(K_DOT_WIDTH_DP, context) * 0.5f;
        this.pointSep = BaseActivity.convertDpToPixel(K_POINT_SEP_DP, context);
        this.hitRadius = BaseActivity.convertDpToPixel(K_HIT_RADIUS_DP, context);

        // create a fake point history to work with
        HistoryValue[] fakeHistory = new HistoryValue[30];
        int index = 0;
        for (; index < 5; ++index) {
            fakeHistory[index] = new HistoryValue(0, null, HistoryValue.Importance.LOW);
        }
        for (; index < 12; ++index) {
            fakeHistory[index] = new HistoryValue(1, null, HistoryValue.Importance.LOW);
        }
        Random random = new Random();
        for (; index < fakeHistory.length; ++index) {
            fakeHistory[index] = new HistoryValue(random.nextBoolean() ? 0 : 1, null, HistoryValue.Importance.LOW);
        }
        setMatchData(fakeHistory);
    }

    public void setMatchData(Match match) {
        if (null != match) {
            setMatchData(match.getWinnersHistory());
        }
    }
    ...
    public boolean setZoomOffset(int newOffset) {
        if (newOffset > 0) {
            ...
            synchronized (this.pointHistory) {
                noPoints = this.pointHistory.size();
            }
    ...
    public void setMatchData(HistoryValue[] winnersHistory) {
        ...
        if (null != drawingRect && 0 != this.pointSep) {
            ...
            if (this.isShowEntireMatchData) {
                this.maxPoints = (int)(winnersHistory.length * (1f / this.viewZoom));
                this.pointSep = drawingRect.width() / Math.max(K_DEFAULT_MAX_POINTS, this.maxPoints);
            }
            else {
                this.pointSep = BaseActivity.convertDpToPixel(K_POINT_SEP_DP, getContext());
        ...
        synchronized (this.pointHistory) {
            ...
            this.touchablePoints = new RectF[winnersHistory.length];
            // add the new data while calculating the data ranges from it
            for (int i = 0; i < winnersHistory.length; ++i) {
                // add the value to our list
                this.pointHistory.add(winnersHistory[i]);
                this.touchablePoints[i] = new RectF(0, 0, 0, 0);
                // and calculate the thresholds
                if (winnersHistory[i].teamIndex == this.teamOnVerticalAxis) {
    ...
    public void setGraphFocus(int teamIndex) {
        if (teamIndex != this.teamOnVerticalAxis) {
            // change the value
            this.teamOnVerticalAxis = teamIndex;
            synchronized (this.pointHistory) {
    ...
    private void recalculateDataOnViewChanges() {
        synchronized (this.pointHistory) {
            ...
            setMatchData(this.pointHistory.toArray(new HistoryValue[0]));
        }
    }
    ...
    protected void onDraw(Canvas canvas) {
        ...
        synchronized (this.pointHistory) {
            ...
            int positiveColor = ContextCompat.getColor(context, this.teamOnVerticalAxis == 0 ? R.color.teamOneColor : R.color.teamTwoColor);
            int negativeColor = ContextCompat.getColor(context, this.teamOnVerticalAxis == 0 ? R.color.teamTwoColor : R.color.teamOneColor);
            ...
            for (int i = this.startPoint + 1; i < this.startPoint + 1 + this.noPoints && i < totalPoints; ++i) {
                ...
                HistoryValue point = this.pointHistory.get(i);
                if (point.teamIndex == this.teamOnVerticalAxis) {
                    color = positiveColor;
                    ++pointSway;
                } else {
                    color = negativeColor;
                ...
                switch (point.importance) {
                    ...
                    case HIGH:
                        ...
                        this.linePaint.setStrokeWidth(BaseActivity.convertDpToPixel(K_IMPR_WIDTH_DP, context));
                        canvas.drawLine(this.drawPoint.x,
                                this.drawPoint.y,
                                this.drawPoint.x,
                                zeroY,
